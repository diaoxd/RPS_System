...
global_dll_path = Path(r'C:\stock\new_tds_test_quant\PYPlugins') / 'TPythClient.dll'
...
class tq:
    ...
    @classmethod
    def _normalize_sector_code(cls, item):
        """将板块代码中的 .1/.0/.2 转为 .SH/.SZ/.BJ"""
        if isinstance(item, str):
            return item.replace('.1', '.SH').replace('.0', '.SZ').replace('.2', '.BJ')
        if isinstance(item, dict):
            code = item.get('code') or item.get('Code') or item.get('value') or ''
            if isinstance(code, str):
                return cls._normalize_sector_code(code)
        return str(item)
    ...
    def get_sector_list(cls) -> List:
        ...
        try:
            ...
            # result = [cls._normalize_sector_code(item) for item in json_res['Value']]
            return result
        except Exception as e:
            cls.close()
    ...
    @classmethod
    def get_sector_list_with_names(cls, fetch_name_by_code: bool = False) -> List[Dict]:
        """
        获取板块列表，并尽量带上板块名称。

        优先使用 DLL 返回中的名称（若存在 Name/name 等字段）；否则仅返回编码，名称为空或等于编码。
        若 fetch_name_by_code=True，会对每个板块代码再请求 get_stock_info 尝试取名称（请求较多，慎用）。

        返回: [{"code": "880301.SH", "name": "概念名"}, ...]
        """
        cls._auto_initialize()
        timeout_ms = 5000
        try:
            ...
            if not ptr or len(ptr) == 0:
                return []
            json_res = json.loads(ptr.decode('utf-8'))
            if json_res.get("ErrorId") != "0":
                return []

            value_list = json_res.get('Value') or []
            # 尝试从接口直接拿到名称：常见为同长度的 Name 列表
            name_list = json_res.get('Name') or json_res.get('name') or json_res.get('Title') or None
            if isinstance(name_list, list) and len(name_list) == len(value_list):
                return [
                    {"code": cls._normalize_sector_code(value_list[i]), "name": str(name_list[i] or value_list[i])}
                    for i in range(len(value_list))
                ]

            # 仅有代码列表（Value 中每项为字符串或 dict）
            result = []
            for item in value_list:
                code = cls._normalize_sector_code(item)
                name = code
                if isinstance(item, dict):
                    name = item.get('name') or item.get('Name') or item.get('title') or code
                result.append({"code": code, "name": name})

            # 若需要且当前名称只是代码，可尝试用 get_stock_info 取板块名称（请求较多）
            if fetch_name_by_code:
                for rec in result:
                    if not rec.get("name") or rec["name"] == rec["code"]:
                        try:
                            info = cls.get_stock_info(rec["code"], field_list=[])
                            for key in ("Name", "StockName", "Name_CN", "股票名称", "name"):
                                if info.get(key):
                                    rec["name"] = str(info[key]).strip()
                                    break
                            if not rec.get("name"):
                                rec["name"] = rec["code"]
                        except Exception:
