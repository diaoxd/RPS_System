...
OUTPUT_FILENAME = "RPS_多周期分析报告_04.html"
    ...
    def build_report_context(self) -> dict:
        ...
        def _top20_by_date(df_sub: pd.DataFrame) -> dict:
            ...
            for d in sorted(df_sub["日期"].unique()):
                ...
                out[d.strftime("%Y-%m-%d")] = [
                    {"name": r["名称"], "type": r["类型"], "avg": round(float(r["avg"]), 1)}
...
class HtmlViewBuilder:
    ...
    def build_report_html(context: dict, default_days: int) -> str:
        ...
        ind["top20_by_date"] = HtmlViewBuilder._inject_chart_file_top20(ind.get("top20_by_date", {}))
        con["top20_by_date"] = HtmlViewBuilder._inject_chart_file_top20(con.get("top20_by_date", {}))
...
<head>
  <meta charset="UTF-8">
  <title>RPS多周期分析报告（通达信数据）04</title>
  <style>
  ...
  function generateTableForPool(tag, dateList) {{
    ...
    dateList.forEach(date => {{
      ...
      if (isTop20) {{
        ...
        topArr.forEach(x => arr.push({{name:x.name, type:x.type, avg:x.avg, chartFile:x.chartFile}}));
      }} else {{
        blocks.forEach(b => {{
          const avg = b.avgData[date];
          if (avg !== null) arr.push({{name:b.name, type:b.type, avg:avg, chartFile:b.chartFile}});
        }});
...
class ReportApplication:
    ...
    def run(self):
        ...
        def _mark(stage: str):
            nonlocal t_prev
            now = time.perf_counter()
            print(f"[TIMER][run] {stage}: +{now - t_prev:.3f}s (total {now - t0:.3f}s)")
            t_prev = now
        ...
        print(f"[TIMER][run] total_elapsed: {time.perf_counter() - t0:.3f}s")
